	...
	return {
		execute: function () {
			...
			exports({
				...
				declareChildApplication: declareChildApplication,
				...
				unloadChildApplication: unloadChildApplication
			});
			...
			var _customEvent_1_0_1_customEvent = useNative() ? NativeCustomEvent :
			...
			function toMountPromise(appOrParcel) {
			  ...
			  return Promise.resolve().then(function () {
			    ...
			    if (!beforeFirstMountFired) {
			      window.dispatchEvent(new _customEvent_1_0_1_customEvent('single-spa:before-first-mount'));
			    ...
			    return reasonableTime(appOrParcel.mount(getProps(appOrParcel)), "Mounting application '".concat(appOrParcel.name, "'"), appOrParcel.timeouts.mount).then(function () {
			      ...
			      if (!firstMountFired) {
			        window.dispatchEvent(new _customEvent_1_0_1_customEvent('single-spa:first-mount'));
			...
			function (_Error) {
			  ...
			  return UserError;
			}(_wrapNativeSuper(Error)); //对每一个注册的资源转为一个promise，
			  ...
			  return Promise.resolve().then(function () {
			    ...
			    return Promise.resolve().then(function () {
			      ...
			      return loadPromise.then(function (val) {
			        ...
			        app.status = NOT_BOOTSTRAPPED; //flatten 扁平化
			...
			function reroute() {
			  ...
			  var wasNoOp = true; //前期注册app时，这里是false，注册完成后，这里变为true，
			  ...
			  function performAppChanges() {
			    return Promise.resolve().then(function () {
			      ...
			      window.dispatchEvent(new _customEvent_1_0_1_customEvent("single-spa:before-routing-event", getCustomEventDetail()));
			  ...
			  function finishUpAndReturn() {
			    ...
			    try {
			      var appChangeEventName = wasNoOp ? "single-spa:no-app-change" : "single-spa:app-change"; //custom-event，基于浏览器的customEvent，兼容了ie8

			      window.dispatchEvent(new _customEvent_1_0_1_customEvent(appChangeEventName, getCustomEventDetail()));
			      window.dispatchEvent(new _customEvent_1_0_1_customEvent("single-spa:routing-event", getCustomEventDetail()));
			    } catch (err) {
